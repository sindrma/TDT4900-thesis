...
Previous user testing has shown that the system is bottlenecked by the number of executing units. The bottleneck was noticed during the Autumn of 2015 when students had to use the system as part of mandatory exercises in the course TDT4200 Parallel Computing \cite{TDT4200}. The students gave feedback at the end of the semester which was presented in the In-Depth Study report delivered in December 2015, and some students pointed out that submissions often stalled for several minutes in the run queue with little feedback on progress. Even though students were advised to start the exercise early and submit to the system long before exercise due date, the system should still handle a higher load of users during the last few hours before the exercise is due. Further, if the system is to be used in programming competitions like \gls{icpc} \cite{ICPC} or IDIOpen \cite{IDIOpen} as well, it needs to handle multiple concurrent submissions without stalling them in the queue for to long.
...
%This section will present related work which can have an effect on the system at a later point. Since no \gls{oj} focusing on energy efficient or heterogenous programming has been found, features of other \gls{ojs} will be presented with focus on unique features. Articles presenting the systems and usage data exists, but are often outdated compared to data presented at the websites of the systems. A combination of both articles and websites are therefore used to gather data.

\subsection{Online Judges and Programming Competitions}
...
\begin{table}[h!]
    \centering
    \begin{tabular}{ l|l }
    \multicolumn{2}{ c }{\textbf{Existing related systems and contests}} \\
    \hline \hline
    \multirow{4}{*}{Programming contest} & Kattis \\
     & UVa Online Judge \\
     & PKU Judge Online \\
     & Green Coding Contest \\ \hline
    \multirow{2}{*}{Crowdsourcing sites} & TopCoder \\
     & RecSys Challenge \\
    \hline
    \end{tabular}
